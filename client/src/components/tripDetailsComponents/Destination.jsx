...
const Destination = memo(
  ({
    ...
    useEffect(() => {
      if (!weatherData) return; // Only update icon when there's weather data
      ...
      setWeatherIconSource(icons[weatherCondition] || icons.Default);
    }, [weatherData]);
    ...
    const handleAddPlace = async (event) => {
      ...
      if (typeof placeInput !== "string" || !placeInput.trim()) {
        toast.error("Place name should be a valid string", {
          theme: "colored",
        });
        return;
      }
    ...
    // Accommodation logic
    const handleSaveAccommodation = useCallback(async (accommodationData) => {
    ...
    // Show more places to visit in Destination
    ...
    // Fetch weather data when needed
