export const validateAccommodationInput = (req, res, next) => {
  ...
  if (!startDate || isNaN(Date.parse(startDate))) {
    return res
      .status(400)
      .json({ success: false, message: "Invalid or missing startDate" });
  }
  ...
  if (!endDate || isNaN(Date.parse(endDate))) {
    return res
      .status(400)
      .json({ success: false, message: "Invalid or missing endDate" });
  }
  ...
  if (bookingLink && typeof bookingLink !== "string") {
    return res
      .status(400)
      .json({ success: false, message: "Invalid or missing booking link" });
  }
